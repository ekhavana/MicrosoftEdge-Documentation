...
| CSP14312 | "Resource violated directive[directive name] in [policy type]: [target uri]. Resource will be blocked." | A  resource violated a directive specified in the `Content-Security-Policy` header and has been blocked from loading. | If the blocked resource is an inline script, either refactor it to an external file and load it from there, or use a nonce or hash to whitelist it as inline code.  |
| CSP14313 | "Resource violated directive [directive name] in [policy type]: [target uri]. Resource will not be blocked due to policy being report-only." | A resource violated the directive specified in the `Content-Security-Policy-Report-Only` header field. Because it is in a `Report-Only` policy type, the resource will not be blocked. | Move the directive into a Content-Security-Policy header field if this resource should be blocked in order to protect your site. |
| CSP14314 | "Failed to enforce sandbox directive in [policy type] because [reason for canceling the operation]." | The sandbox directive failed for the reasons specified. This directive places restrictions on actions the page can take, rather than on resources that the page can load. If the sandbox directive is present, the page will be treated as though it was loaded inside of an iframe. It can prevent popups, plugins and block script execution. | Keep the sandbox value empty to keep all restrictions in place, or add values: `allow-forms` `allow-same-origin` `allow-scripts`, and `allow-top-navigation`. **The sandbox directive is not supported in the `<meta>` element or in the `Content-Security-policy-Report-Only` header field.* |
| CSP14315 | "Script eval being allowed due to host override." | When either the `script-src` or the `default-src` directive is included, inline script and `eval()` are disabled unless you specify 'unsafe-inline' and 'unsafe-eval', respectively. | `'unsafe-eval'` allows the use of `eval()` and similar methods for creating code from strings. You must include the single quotes. *Note: This is unsafe and can open your web site up to cross-site scripting vulnerabilities.* |
| CSP14316 | "Frame [source name] being allowed due to host override." | The source frame identified has been allowed due to an override set in the host CSP policy. | A policy may contain a nonce-source expression, meaning that the source can only be used on one occasion and the server must generate a fresh value for the directive each time it transmits a policy. Nonces override the restrictions in the directive in which they are present. |
| CSP14317 | "Ignoring 'unsafe-inline' for directive [directive name] in [policy type] because nonce or hash value is specified." | The user agent will block the execution of any inline script or styles that are not whitelisted via nonce or hashing. | A document requiring inline script execution might specify `script-src 'unsafe-inline' 'nonce-myNonceValue'` so that user agents supporting CSP Level 2 will apply the stricter policy of only executing inline `<script nonce='myNonceValue'>` blocks, and user agents that support CSP 1.0 will continue to allow the execution of all inline script. |
| CSP14318 | "The blocked resource has a hash of [hash value]." | If a CSP directive has a specified hash source, any applicable resources that are blocked will report their hashes through this warning. | Double check you only included the element content, and not the surrounding `<script>` or `<style>` tags in the expected hash. |
| CSP14319 | "Plugin type [MIME type] (declared type matches real type: [true/false]) violated directive [directive name]  in [policy type]. Resource will be blocked." | The document attempted to load a plugin explicitly disallowed by the current CSP and was blocked in doing so. | Verify your intended plugin element's `type` attribute declares a valid MIME type and that it's correctly listed in the `plugin-types` directive.  |
| CSP14320 | "Plugin type [MIME type] (declared type matches real type: [true/false]) violated directive [directive name]  in [policy type]. Resource will not be blocked due to policy being report-only." | The document attempted to load a plugin that would be otherwise disallowed if the directive were in CSP mode (and not report-only).  | Move the directive into a Content-Security-Policy header field if this plugin should be blocked.  |
| CSP14321 | "Resource violated directive [directive name] in [policy type], in [target uri] at line [number] column [number]. Resource will be blocked." | A  resource violated a directive specified in the `Content-Security-Policy` header and has been blocked from loading. |  If the blocked resource is an inline script, either refactor it to an external file and load it from there, or use a nonce or hash to whitelist it as inline code. |
| CSP14322 | "Resource violated directive [directive name] in [policy type], in [target uri] at line [number] column [number]. Resource will not be blocked due to policy being report-only." | A resource violated the directive specified in the `Content-Security-Policy-Report-Only` header field. Because it is in a `Report-Only` policy type, the resource will not be blocked. | Move the directive into a Content-Security-Policy header field if this resource should be blocked in order to protect your site. |
